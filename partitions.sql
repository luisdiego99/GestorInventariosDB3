-- Reporte de productos por id_store 
-- Drop table stores;
...
select *from stores partition(p0);

-- Tabla particion vertical de tabla products
-- Tabla original
Drop table stores;
CREATE TABLE IF NOT EXISTS stores (
    id_store INT AUTO_INCREMENT PRIMARY KEY,
...
-- informacion basica tabla products    
...
CREATE TABLE IF NOT EXISTS products_details (
    ...
    FOREIGN KEY (id_product) REFERENCES products_basic(id_product) ON DELETE CASCADE ON UPDATE CASCADE
);
SELECT pb.id_product, pb.sku_number, pb.product_name, pb.id_store, pb.stock, pd.descriptions, pd.unit_price, pd.store_location
FROM products_basic pb
JOIN products_details pd ON pb.id_product = pd.id_product;


-- Reporte de productos por id productos vendidos, cantidad, fecha, y valor total.
-- particion vertical de tabla transaction
-- Tabla original
...
-- informacion basica tabla transactions
CREATE TABLE IF NOT EXISTS transactions_basic (
    id_transaction INT AUTO_INCREMENT PRIMARY KEY,
    date_transaction DATETIME,
    id_product INT,
    quantity INTEGER,
...
-- detalles tabla transactions
CREATE TABLE IF NOT EXISTS transactions_details (
    id_transaction INT PRIMARY KEY,
	id_user INT,
    type_transaction VARCHAR(25),
    FOREIGN KEY (id_user) REFERENCES users(id_user) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (id_transaction) REFERENCES transactions_basic(id_transaction) ON DELETE CASCADE ON UPDATE CASCADE
);

SELECT tb.id_transaction, tb.date_transaction, tb.id_product, tb.quantity, tb.subtotal
FROM transactions_basic tb
JOIN transactions_details td ON tb.id_transaction = td.id_transaction;
